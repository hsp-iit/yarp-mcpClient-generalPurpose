...
class Yarp_mcpClient_GeneralCheckerCore:
                    ...
                    async with streamablehttp_client(server_info["url"]) as (read_stream, write_stream, get_session_id):
                        async with ClientSession(read_stream, write_stream) as session:
                            ...
                            for tool in tools_response.tools:
                                ...
                                # Extract monitoring metadata if provided by server
                                # Servers can add "x-monitoring" property to indicate monitoring capabilities
                                if isinstance(input_schema, dict) and "x-monitoring" in input_schema:
                                    monitoring_data = input_schema["x-monitoring"]
                                    tool_info["monitoring"] = monitoring_data
                                    # Also store in central monitoring_metadata dict
                                    self.monitoring_metadata[tool.name] = monitoring_data
                                    logger.debug(f"Found monitoring metadata for {tool.name} in schema: {monitoring_data}")

                                # Also try to extract from docstring as a fallback
                                # Servers may include x-monitoring metadata in the tool description
                                elif tool.description and "x-monitoring" in tool.description:
                                    try:
                                        # Try to extract JSON from the description
                                        import re
                                        json_match = re.search(r'"x-monitoring":\s*({[^}]+})', tool.description)
                                        if json_match:
                                            monitoring_json = json_match.group(1)
                                            # Replace single quotes with double quotes for JSON parsing
                                            monitoring_json = monitoring_json.replace("'", '"')
                                            monitoring_data = json.loads(monitoring_json)
                                            tool_info["monitoring"] = monitoring_data
                                            self.monitoring_metadata[tool.name] = monitoring_data
                                            logger.debug(f"Found monitoring metadata for {tool.name} in docstring: {monitoring_data}")
                                    except Exception as e:
                                        logger.debug(f"Could not parse x-monitoring from {tool.name} description: {e}")

                                descriptions[tool.name] = tool_info
...
"""
        ...
        if monitoring_tools:
            ...
            prompt += """You have access to background monitoring tools that allow you to:
...
"""
        ...
        prompt += """
═════════════════════════════════════════════════════════════════════════════════
GENERAL GUIDELINES FOR USING TOOLS AND MONITORING:
═════════════════════════════════════════════════════════════════════════════════
...
═════════════════════════════════════════════════════════════════════════════════"""
    ...
    async def process_user_message(self, user_input: str) -> str:
        ...
        while iteration < max_iterations:
            ...
            # Prepare messages for the LLM (include system prompt if this is the first user message)
            messages = []
            messages.append({"role": "system", "content": self.system_prompt})
            # if len([msg for msg in self.conversation_history if msg["role"] == "user"]) == 1:
            #     messages.append({"role": "system", "content": self.system_prompt})
            #     logger.info(f"Adding system prompt to first message. Prompt length: {len(self.system_prompt)} chars")
            ...
            try:
                ...
                if message.tool_calls:
                    ...
                    for tool_call in message.tool_calls:
                        ...
                        print(f"{Colors.OKCYAN}🔧 Calling tool: {fn_name}({fn_args}){Colors.ENDC}")
                        ...
                        if fn_name == "start_monitoring":
                            ...
                            # Call the MCP tool
                            result = await self.call_mcp_tool(fn_name, fn_args, server_url)
